...
class HunspellXMLExporter
{
	...
	static final defaultOptions = [hunspell:true,
		tests:true,
		thesaurus:true,
		license:true,
		readme:true,
	...
	def options = defaultOptions.clone()
		
	public HunspellXMLExporter(HunspellXMLData data, Log log, optionDiff)
	{
		this.log = log
		this.data = data
		optionDiff.each{key,val-> options[key] = val}
	}
	...
	public HunspellXMLExporter(HunspellXMLData data, Log log)
	{
		...
		public HunspellXMLExporter(HunspellXMLData data, Map optionDiff)
	{
		this.log = log
		this.data = data
		optionDiff.each{key,val-> options[key] = val}
	...
	def export()
	{
		...
		baseDir = configureBaseDir(data, options.customPath, filename)
	...
	static String configureBaseDir(HunspellXMLData data, String customPath="", String filename)
	{
		def baseDir = ""
	...
	def createTestFiles()
	{
		goodFile = baseDir + FS + filename + "_good.txt"
		def file = new File(goodFile)
		file.withWriter(data.metadata.javaEncoding){writer->
			writer << data.goodTestFile
		}
		
		badFile = baseDir + FS + filename + "_bad.txt"
	...
	def createLibreOfficePlugin()
	{
		...
		new File(tempDir + FS + "META-INF").mkdir()
		new File(tempDir + FS + "dictionaries").mkdir()
...
new File(tempDir + "/dictionaries.xcu").withWriter("UTF-8"){writer->
writer << """<?xml version="1.0" encoding="UTF-8"?>
...
"""
			...
			new File(baseDir + FS + "LibreOffice").mkdir()
...
"""
			...
			new File(baseDir + FS + "Firefox").mkdir()
...
"""
			...
			new File(baseDir + FS + "Opera").mkdir()
